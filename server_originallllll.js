...
// app.post('/checkin', (req, res) => {
//     const { name, from, email, phone, employee_id, health_safety } = req.body;
...
//     // Assuming you want to store "Yes" as 1 and "No" as 0 in the database
//     const healthSafetyValue = (health_safety.toLowerCase() === 'yes') ? 1 : 0;

//     // Handle the case when employee_id is empty
//     const employeeIdValue = 28 || /* Provide a default employee_id here */ 'DEFAULT_VALUE';

//     conn.query(sql, [name, from, email, phone, employeeIdValue, healthSafetyValue], (err, result) => {
//         if (err) {
//             console.error('Error inserting visitor data:', err);
//             return res.status(500).send(`Error inserting visitor data: ${err.message}`);
//         }
...
//     var sql = `INSERT INTO visitors (name, from, email, phone, employee_id, health_safety) VALUES (?, ?, ?, ?, ?, ?)`;

//     if (name && phone) {
//         conn.query(sql, [name, from, email, phone, employee_id, health_safety], function (err, result) {
//             if (err) {
//                 console.error(err);
//                 res.status(500).send('Internal Server Error');
//             } else {
